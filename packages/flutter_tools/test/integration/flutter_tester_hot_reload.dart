  ...
  group('FlutterTesterDevice', () {
    ...
    testUsingContext('can reload if breakpoints have file:// prefixes', () async {
      ...
      try {
        ...
        final VMIsolate isolate = await (await vmService.getVM()).isolates.first.loadRunnable();
        final Uri breakpoint = new Uri.file(fs.path.join(_tempDir.path, 'lib', 'main.dart'));
        await isolate.addBreakpoint(breakpoint.toString(), 7);
        await isolate.resume(); // Start the app
        await isolate.waitUntilPaused(); // Hit the breakpoint
        await isolate.resume(); // Continue
